...
class ConsoleHscript {
	...
	private function prepareString(str:String) {
		if (str.endsWith(";")) str = str.substr(0, -1);
		return str;
	}
	private function getObj(str:String) {
		var obj:Dynamic = null;
		try {
			obj = interp.expr(parser.parseString(str, "console"));
		} catch(e:Error) {
			//_errorHandler(e); //ignore errors while getting fields
		} catch(e) {
			//trace(e.toString());
		}
		return obj;
	...
	private function getFieldTypeName(obj:Dynamic) {
	...
	private function getFieldTypeValue(obj:Dynamic) {
		switch(Type.typeof(obj)) {
			case TBool:
			...
			case TClass(c):
				if (getFieldTypeName(obj) == "Array" && obj != null) {
					return "Length: " + obj.length + (obj.length > 0 ? ", " + getFieldTypeName(obj[0]) : "");
